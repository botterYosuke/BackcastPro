...
from BackcastPro.api.gdrive_client import GDriveConfig, GDriveClient


class TestGDriveConfig(unittest.TestCase):
    ...
    def test_from_environment_with_values(self):
        ...
        with patch.dict(os.environ, {
            ...
            config = GDriveConfig.from_environment()
    ...
    def test_is_configured_true(self):
        """URLがある場合はTrue"""
        config = GDriveConfig(api_base_url='https://my-api.run.app')
        self.assertTrue(config.is_configured())

    def test_is_configured_false(self):
        """URLが空の場合はFalse"""
        config = GDriveConfig(api_base_url='')
        self.assertFalse(config.is_configured())
...
class TestGDriveClient(unittest.TestCase):
    ...
    def setUp(self):
        self.config = GDriveConfig(api_base_url='https://my-api.run.app')
        self.client = GDriveClient(self.config)
    ...
    def test_download_file_success(self):
        ...
        with patch('BackcastPro.api.gdrive_client.requests.get', return_value=mock_resp):
    ...
    def test_download_file_connection_error(self):
        """接続エラー"""
        with patch('BackcastPro.api.gdrive_client.requests.get',
    ...
    def test_download_file_url_construction(self):
        ...
        with patch('BackcastPro.api.gdrive_client.requests.get', return_value=mock_resp) as mock_get:
            with tempfile.NamedTemporaryFile(delete=False) as f:
                test_path = f.name
