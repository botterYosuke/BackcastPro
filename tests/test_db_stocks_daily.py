...
class TestDbStocksDaily(unittest.TestCase):
    ...
    def test_metadata_operations(self):
        ...
        try:
            ...
            self.assertEqual(
                str(meta["to_date"]), "2024-01-20"
            )  # Should update to newest
    ...
    def test_check_period_coverage(self):
        ...
        res = self.db_daily._check_period_coverage(
            metadata, datetime(2024, 1, 5), datetime(2024, 1, 10)
        )
        self.assertTrue(res["is_covered"])

        # Case 2: Partially covered (start before)
        res = self.db_daily._check_period_coverage(
            metadata, datetime(2023, 12, 31), datetime(2024, 1, 10)
        )
        self.assertFalse(res["is_covered"])

        # Case 3: Partially covered (end after)
        res = self.db_daily._check_period_coverage(
            metadata, datetime(2024, 1, 20), datetime(2024, 2, 1)
        )
        self.assertFalse(res["is_covered"])

        # Case 4: Not covered (disjoint)
        res = self.db_daily._check_period_coverage(
            metadata, datetime(2024, 2, 1), datetime(2024, 2, 10)
        )
        self.assertFalse(res["is_covered"])
    ...
    def test_save_and_load_stock_prices(self):
        ...
        try:
            ...
            meta_count = con.execute(
                'SELECT record_count FROM stocks_daily_metadata WHERE "Code"=?', [code]
            ).fetchone()[0]
            self.assertEqual(meta_count, 2)

            # Check date range in metadata
            dates = con.execute(
                'SELECT from_date, to_date FROM stocks_daily_metadata WHERE "Code"=?',
                [code],
            ).fetchone()
        ...
        self.assertEqual(
            pd.to_datetime(loaded_df_filtered.index[0]), datetime(2024, 1, 2)
        )
