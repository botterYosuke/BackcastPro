...
class e_api:
    ...
    def _load_from_cache(self) -> bool:
        ...
        try:
            ...
            token_expires_at = datetime.fromisoformat(
                cache_data.get("token_expires_at")
            )
    ...
    def _save_to_cache(self) -> None:
        ...
        try:
            if self.token_expires_at is None:
                logger.warning(
                    "token_expires_atがNullのため、キャッシュに保存できません。"
                )
                return
    ...
    def _load_login_failures(self) -> None:
        ...
        try:
            ...
            self.login_failures = [
                datetime.fromisoformat(dt) for dt in failure_data.get("failures", [])
            ]
            if failure_data.get("blocked_until"):
                self.login_blocked_until = datetime.fromisoformat(
                    failure_data.get("blocked_until")
                )
    ...
    def _save_login_failures(self) -> None:
        ...
        try:
            ...
            failure_data = {
                ...
                "blocked_until": self.login_blocked_until.isoformat()
    ...
    def _record_login_failure(self) -> None:
        ...
        self.login_failures = [
            dt for dt in self.login_failures if dt > twenty_four_hours_ago
        ]
        ...
        if len(self.login_failures) >= self.max_login_failures:
            ...
            logger.warning(
                f"24時間以内に{self.max_login_failures}回ログインに失敗しました。{self.login_blocked_until}までログインをブロックします。"
            )
    ...
    def _is_login_blocked(self) -> bool:
        ...
        if self.login_blocked_until:
            if datetime.now() < self.login_blocked_until:
                logger.warning(
                    f"ログインがブロックされています。ブロック解除: {self.login_blocked_until}"
                )
                return True
            else:
                # ブロック期間が過ぎたらリセット
    ...
    def _set_token(self) -> bool:
        ...
        try:
            ...
            str_api_response = req.content.decode(
                req.apparent_encoding, errors="ignore"
            )
            ...
            try:
                ...
                if (
                    int_p_errno == 0 and int_sResultCode == 0
                ):  # ログインエラーでない場合
                    # 仮想URLを保存する。
                    ...
                    token_datetime = datetime.strptime(
                        p_sd_date, "%Y.%m.%d-%H:%M:%S.%f"
                    )
                ...
                logger.error(
                    f"立花証券-e支店のログイン情報の保存の取得に失敗しました: {e}"
                )
                self._record_login_failure()
        return False
    ...
    def get_daily_quotes(
        ...
        if from_ is not None:
            ...
            if from_ < twenty_years_ago:
                logger.info(
                    f"from_パラメータ({from_})が20年より前のため、スキップします。"
                )
                return pd.DataFrame()
