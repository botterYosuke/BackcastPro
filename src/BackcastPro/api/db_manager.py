...
class db_manager:
    ...
    def __init__(self):
        ...
        if not os.path.exists(self.cache_dir):
            try:
                ...
                logger.warning(
                    f"キャッシュディレクトリの作成に失敗しました: {self.cache_dir}, エラー: {e}"
                )
                self.isEnable = False
        else:
            self.isEnable = True
        ...
        if not self.isEnable:
            logger.warning(
                f"キャッシュディレクトリが存在しないか、アクセスできません: {self.cache_dir}"
            )

    def _table_exists(
        self, db_connection: duckdb.DuckDBPyConnection, table_name: str
    ) -> bool:
        """テーブルが存在するかチェック"""
        try:
        ...
        if not self._table_exists(db_connection, table_name):
            ...
            try:
                ...
                if self._table_exists(db_connection, table_name):
                    logger.info(
                        f"テーブル '{table_name}' は既に存在します（他のスレッドが作成した可能性）"
                    )
                else:
                    # それ以外のエラーは再スロー
                    raise e
    ...
    def __add_db__(
        ...
        existing_count = db_connection.execute(
        ...
        if existing_count == 0:
            ...
            if key in df.columns:
                ...
                existing_disclosure_numbers = set(
                    db_connection.execute(f"SELECT DISTINCT {key} FROM {table_name}")
                    .fetchdf()[key]
                    .astype(str)
                )
                ...
                # 新規のkeyのみを抽出
                unique_disclosure_numbers = (
                    new_disclosure_numbers - existing_disclosure_numbers
                )

                if unique_disclosure_numbers:
                    # 新規データのみをフィルタリング
                    new_data_df = df[
                        df[key].astype(str).isin(unique_disclosure_numbers)
                    ]
    ...
    def _validate_table_schema(
        ...
        try:
            ...
            if missing_columns or extra_columns:
                warning_msg = (
                    f"カラム構造の不一致を検出しました - テーブル: {table_name}"
                )
                if missing_columns:
    ...
    def _batch_insert_data(
        ...
        if total_rows <= batch_size:
            ...
            # 大量データはチャンクに分割して挿入
            logger.info(
                f"大量データをバッチ処理で挿入します: {total_rows}件 (バッチサイズ: {batch_size})"
            )
            ...
            for i in range(0, total_rows, batch_size):
                ...
                logger.debug(
                    f"バッチ挿入進捗: {processed}/{total_rows} 件 ({processed / total_rows * 100:.1f}%)"
                )
    ...
    def ensure_db_ready(self, code: str = None) -> None:
        ...
        if not os.path.exists(db_path):
            ...
            if self._download_from_cloud(db_path, normalized_code):
                logger.info(
                    f"DuckDBファイルをクラウドからダウンロードしました: {db_path}"
                )
            else:
