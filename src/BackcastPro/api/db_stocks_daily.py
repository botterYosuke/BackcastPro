                ...
                try:
                    if self._table_exists(db, table_name):
                        ...
                        if unique_pairs:
                            mask = df_to_save_copy.apply(
                                lambda row: (
                                    (str(row["Code"]), str(row["Date"])) in unique_pairs
                                ),
                                axis=1,
                            )
                        ...
                        if not self._table_exists(db, table_name):
                            ...
                            primary_keys = (
                                ...
                                and "Date" in df_to_save_normalized.columns
                                else ["Date"]
                            )
                    ...
                    if "Date" in df_to_save.columns:
                        ...
                        if date_stats and date_stats[0]:
                            ...
                            self._save_metadata(
                                db, code, actual_from, actual_to, actual_count
                            )

                    # トランザクションコミット
                    db.execute("COMMIT")
                    logger.info(
                        f"priceデータをDuckDBに保存しました: 銘柄コード={code}, 件数={len(df_to_save)}"
                    )
    ...
    def load_stock_prices_from_cache(
        ...
        try:
            ...
            with self.get_db(code) as db:
                if not self._table_exists(db, table_name):
                    logger.debug(
                        f"キャッシュにデータがありません（外部APIから取得します）: {code}"
                    )
                    return pd.DataFrame()
                ...
                logger.info(
                    f"株価データをDuckDBから読み込みました: {code} ({len(df)}件)"
                )
